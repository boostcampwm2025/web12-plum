...
describe('useMediaProducer', () => {
  ...
  describe('produce', () => {
    it('유효하지 않은 트랙 상태면 에러를 던져야 한다', async () => {
      ...
      await expect(
        result.current.produce(mockSocket as never, invalidTrack as never),
    ...
    it('Transport가 없으면 새로 생성해야 한다', async () => {
      ...
      await act(async () => {
        await result.current.produce(mockSocket as never, mockTrack as never);
      });
    ...
    it('appData.type이 없으면 기본값 camera를 사용해야 한다', async () => {
      const mockProducer = createMockProducer();
      mockTransport.produce.mockResolvedValue(mockProducer);
      ...
      expect(mockTransport.produce).toHaveBeenCalledWith(
        expect.objectContaining({
          appData: expect.objectContaining({ type: 'camera' }),
        }),
      );
    });
    ...
    it('동일한 type의 Producer가 있지만 트랙 ID가 다르면 기존 것을 닫고 새로 생성해야 한다', async () => {
      ...
      // 첫 번째 produce
      await act(async () => {
        await result.current.produce(mockSocket as never, oldTrack as never);
      });

      // 다른 트랙으로 두 번째 produce
      await act(async () => {
        await result.current.produce(mockSocket as never, newTrack as never);
      });
    ...
    it('transport.produce 실패 시 에러를 전파해야 한다', async () => {
      ...
      await expect(result.current.produce(mockSocket as never, mockTrack as never)).rejects.toThrow(
        'Produce failed',
      );
    });
  });
  ...
  describe('stopProducing', () => {
    it('지정된 kind의 Producer만 중단해야 한다', async () => {
      const videoProducer = createMockProducer({ kind: 'video', appData: { type: 'camera' } });
      const audioProducer = createMockProducer({ kind: 'audio', appData: { type: 'mic' } });
      ...
      // 두 개의 Producer 생성
      await act(async () => {
        await result.current.produce(mockSocket as never, videoTrack as never, { type: 'camera' });
        await result.current.produce(mockSocket as never, audioTrack as never, { type: 'mic' });
      });
    ...
    it('stopProducing 후 activeProducers가 업데이트되어야 한다', async () => {
      ...
      await act(async () => {
        await result.current.produce(mockSocket as never, videoTrack as never);
      });
  ...
  describe('getProducer', () => {
    it('존재하는 type의 Producer를 반환해야 한다', async () => {
      const mockProducer = createMockProducer({ appData: { type: 'camera' } });
      mockTransport.produce.mockResolvedValue(mockProducer);
      ...
      await act(async () => {
        await result.current.produce(mockSocket as never, mockTrack as never, { type: 'camera' });
      });

      expect(result.current.getProducer('camera')).toBe(mockProducer);
    });

    it('존재하지 않는 type의 경우 null을 반환해야 한다', () => {
      const { result } = renderHook(() => useMediaProducer());

      expect(result.current.getProducer('nonexistent')).toBeNull();
    });

    it('type 미지정 시 기본값 camera로 조회해야 한다', async () => {
      const mockProducer = createMockProducer({ appData: { type: 'camera' } });
      mockTransport.produce.mockResolvedValue(mockProducer);
  ...
  describe('이벤트 핸들러 동작', () => {
    it('transportclose 이벤트 발생 시 Producer가 정리되어야 한다', async () => {
      const mockProducer = createMockProducer({ kind: 'video', appData: { type: 'camera' } });
      mockTransport.produce.mockResolvedValue(mockProducer);
