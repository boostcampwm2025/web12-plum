...
const ALLOWED_GESTURES = [
    'Thumb_Up', 'Thumb_Down', 'Open_Palm', // MediaPipe 기본 제스처
    'ok_sign', 'number_1', 'number_2', 'number_3', 'number_4' // 커스텀 제스처
];

// 제스처 이름과 해당 제스처를 나타내는 이모지 아이콘 매핑입니다.
// UI에서 현재 감지 중인 제스처를 시각적으로 보여줄 때 사용됩니다.
const GESTURE_ICONS = {
    "Open_Palm": "🖐️", "Thumb_Down": "👎", "Thumb_Up": "👍",
    "ok_sign": "👌",
    "number_1": "1️⃣", "number_2": "2️⃣", "number_3": "3️⃣", "number_4": "4️⃣"
};
...
let currentGestureState = { // 현재 감지 중인 (아직 확정되지 않은) 제스처의 상태
  name: null,       // 제스처 이름
...
const enableCam = () => {
  ...
  if (webcamRunning) {
    // 웹캠 활성화 시, 사용자 미디어(비디오) 스트림을 요청합니다.
    navigator.mediaDevices.getUserMedia({ video: true }).then((stream) => {
      video.srcObject = stream; // <video> 요소에 스트림을 연결합니다.
      // 비디오 데이터가 모두 로드되면 예측 루프를 시작합니다.
      video.addEventListener("loadeddata", predictWebcam);
    });
    webcamButton.style.display = 'none'; // 웹캠 활성화 후 버튼을 숨깁니다.
  } else {
...
async function predictWebcam() {
  ...
  if (now - lastProcessTime > 300) {
    ...
    if (customGesture) {
        ...
        score = 0.9; // 커스텀 제스처는 신뢰도 90%로 임의 설정 (실제 모델 점수가 없으므로)
    } else {
